...
def test_malformed_session_id_is_a_404_not_a_500(client, path):
    r = client.get(path)
    assert r.status_code == 404, (path, r.status_code, r.text[:120])
    assert "not found" in r.json()["detail"]


def test_delete_on_a_word_that_is_not_a_session_is_a_404(client):
    # schemathesis reached DELETE /assist/{session_id} with "candidates"
    assert client.delete("/assist/candidates").status_code == 404
