...
|---|---|---|
| Phase 4 (HIGH #17) | All 5 OWUI pipelines call bare `requests.get/post` instead of a module-level `Session()` | **Demoted from HIGH to LOW during cluster K verification.** Anti-pattern, not measured perf issue. Each pipeline issues one orchestrator call per user message; OWUI traffic is human-paced (~1 cmd/min). At ~10 ms saved per call from connection reuse, even a 100-message session saves ~1s aggregate — not HIGH-tier. Refactoring is also non-trivial: tests patch `requests.get`/`requests.post` at 36+ sites (e.g. `tests/test_scaffold_router_commands.py`) and switching to `Session` would break those mocks. Re-classified as LOW; carried forward as a separate refactoring task rather than blocking this cluster. |
| Phase 4 (HIGH #15) | API-key drift between `valves.json` and `SCAFFOLD_API_KEY` warned via `print()` only, invisible in OWUI UI | **Partially fixed in cluster K.** scaffold_router (the most user-facing pipeline) now sets `_api_key_drift_detected` at init and exposes a `_drift_hint()` markdown block that gets appended to user-visible 401 error responses in `_handle_confirm`. Drift surfaces exactly when the user hits an auth failure. Other 4 pipelines (execution_handler, dag_viewer, gt_browser, prompt_inspector) still print-only — same pattern, lower call volume — flagged as future LOW work. Original "SEC HIGH" framing was overstated: the drift IS captured in container logs (ops surface), so this is a UX gap, not a security boundary. |
| Phase 4 (HIGH #16) | `pipelines/scaffold_router.py:1460` `timeout=(30, 120)` aborts long SSE streams | **Retracted during cluster J verification.** The 120s is a *read-poll interval*, not a stream-abort timeout. `requests.iter_lines()` raises `ReadTimeout` after 120s of no data; the handler at L1506-1517 catches it, increments the idle counter by `keepalive_interval` (default 10s), emits a heartbeat, and continues. The stream is only declared stalled after `idle_seconds >= max_idle = max(300, 5*keep)`. `/research` and `/execute/all` are NOT aborted by this timeout. Real adjacent issue: `idle_seconds += keep` undercounts real elapsed time (each cycle is 120s wall, not 10s), so the effective stall threshold is ~12× larger than `max_idle` suggests — flagged for future work as a counter calibration, not a HIGH. |
| Phase 3c MED cluster M5 | `execution_compile.py:36` "title heuristic", `execution_compile` "no compile_output_gate_chars enforcement", `execution_agent.py:707/908` "retry-loop cache stale" | **Cluster M5 fully retracted during verification.** L36 uses `tool == "CodeGen"` (an intentional fallback strategy when no node is marked `is_output_node`, per the file's own docstring), not a title heuristic. The compile-gate is correctly applied at the SSE display boundary (L907-910); pushing it into `_compile_output` would also cap the persisted DB row, which is the canonical deliverable. There is no in-memory cache between retries — `_build_pipeline_summary` reads fresh from DB on every call. The "L530-595 unwrapped" carry-forward from cluster D is also speculative: `_fetch_rag_context`, `_searxng_search`, and `_milvus_search` each catch `Exception` internally and return strings. No fix applied. |
