...
**Last Updated:** April 10, 2026 (post-audit)
...
User types plain message
  │
  ▼
Triage conversation (qwen2.5:7b via Ollama)
...
/confirm triggers auto-chain (Phase 2 → DAG → Execute):
  ...
  └── Execution: POST /execute/all (SSE streaming)
        ...
        ├── Failed nodes are skipped; blocked downstream nodes reported
...
After each node runs, the verifier model (qwen2.5:7b) checks the output before moving on. If a node fails verification, it can be retried via `/exec/retry` or skipped with `/skip <job_id> <node_key>`.

DAG truncation enforces a maximum node count (currently 5). When the LLM generates more nodes than allowed, the truncator keeps the first N by **numeric sort** (T1, T2, T3...) and removes references to dropped nodes from remaining dependency lists.
...
- `scaffold_router.py` DAG timeout: configurable via `dag_timeout` valve (default **900s**)
...
| Triage | `qwen2.5:7b` | `triage_model` valve in scaffold_router.py | Conversational triage + idea synthesis |
...
| File | Lines | Purpose |
|------|-------|---------|
| `app/modules/execution_agent.py` | ~1,139 | DAG node execution, SSE streaming, tool dispatch, verification, compiled output, concurrent execution guard, upstream prompt restructuring. Uses short-lived database sessions |
| `app/modules/dag_generator.py` | ~615 | DAG creation with Kahn's cycle detection, numeric-sort truncation. JSON parsing via shared `llm_parsing.py` |
...
| `dag_timeout` | `900` | Seconds to wait for DAG generation |
| `keepalive_interval` | `10` | Seconds between keepalive zero-width spaces |
| `triage_model` | `qwen2.5:7b` | Model for conversational triage and synthesis |
...
9. **Conversational triage** — Lightweight `qwen2.5:7b` handles multi-turn planning before the expensive pipeline runs
10. **Two-model pipeline routing** — Triage uses 7b (fast), full pipeline uses 235b (deep)
...
1. **Triage model latency on long conversations** — `qwen2.5:7b` on CPU can take several minutes per turn as context grows. Consider switching to `qwen3:4b` if consistently problematic
2. **`/ideate/confirm` returned 500** on one occasion — root cause unknown, may be transient Milvus/LLM error
3. **DAG node cap at 5** — LLM generates 8–10 nodes but truncator caps at 5. Consider raising cap or smarter truncation
4. **No automatic retry** — Failed nodes require manual `/exec/retry` or `/skip`. No auto-retry logic
5. **End-to-end pipeline under active testing** — Full auto-chain functional but still being validated on CPU-only hardware
...
|-----------|------------------|-------|
| Triage turn (qwen2.5:7b) | ~30s – 300s+ | Scales with conversation length |
| Idea synthesis (qwen2.5:7b) | ~30s – 120s | Depends on conversation length |
...
│   ├── modules/
│   │   ├── execution_agent.py     # DAG execution, SSE streaming
