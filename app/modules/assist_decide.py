...
_DECIDE_EXTRA = (
    ...
    "- They report FINISHING this step — a result, 'done', 'that worked', a clean "
    "success paste with no error → submit. This RECORDS what they did as the "
    "step's output AND moves on; put their result (or a brief 'confirmed working') "
    "in `evidence`. Do NOT use advance for a completion — advance neither records "
    "nor marks the step done, so it just re-shows the same step. Use advance ONLY "
    "to move to the next step with nothing to record here (a bare 'next', or they "
    "already handled this elsewhere).\n"
