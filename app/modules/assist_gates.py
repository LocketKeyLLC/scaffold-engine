...
GATES: tuple[Gate, ...] = (
    ...
    Gate("fact_plan_trigger", "record", "assist_plan_facts", "blocking_candidates", (FACT_WRITE,), "§17.1089",
         "a fact that blocks a pending step (absent/gated/unsupported capability) is judged for a re-plan proposal"),
    Gate("fact_satisfies_step", "record", "assist_plan_facts", "satisfied_candidates", (FACT_WRITE,), "§17.1104",
         "a fact showing a pending step's outcome is already achieved → verify and complete it (plan reconciliation)"),
    Gate("fact_obsoletes_step", "record", "assist_plan_facts", "obsolete_candidates", (FACT_WRITE,), "§17.1104",
         "a fact that reversed a pending step's basis (reverted/replaced/changed) → propose dropping it"),
