...
Do NOT execute anything. Do NOT invent requirements the user hasn't agreed to."""
    ...
    def pipe(
        ...
        # Strip Open WebUI context injection to find the real user message
        import re as _re
        _ctx_match = _re.search(r'</context>\s*
*(.*)', msg, _re.DOTALL)
        real_msg = _ctx_match.group(1).strip() if _ctx_match else msg
        print(f"DEBUG user_message repr: {repr(msg[:200])}")
        print(f"DEBUG real_msg repr: {repr(real_msg[:200])}")
        msg = real_msg
